strList = """seg_route_from has constant value "23432" 	Constant
...
strList2 = """ffp_nbr has 19021 (81.2%) zeros 	Zeros
...
List3 = []
for i in List:
    List2.append(i.split(" has")[0])
    k = (i.split("(")[1]).split("%")[0]
    if len(k.split(" "))>1:
        k = k.split(" ")[1]
    List3.append(float(k))



StrList = dict(zip(List2,List3))
# print(StrList)

StrList = sorted(StrList.items(), key = lambda kv:(kv[1], kv[0]))

Sum = 0
StrLL = ""
for i in StrList:
    if i[1]>95 :
        StrLL= StrLL+i[0]+":"+str(i[1])+"\t"
        Sum+=1
print(Sum)
print(StrLL)